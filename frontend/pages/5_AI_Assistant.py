...
Phase 4 will connect these to Claude via MCP tools mounted at /mcp.
"""
import sys
...
st.set_page_config(page_title="AI Assistant | PM Tracker", page_icon="🤖", layout="wide")
st.title("🤖 AI Project Assistant")
st.caption(
    "Automated analysis powered by the CPM and EVM engine. "
    "Phase 4 will upgrade these to Claude AI via FastMCP at `/mcp`."
)
...
if st.button("Run Risk Analysis", type="primary", key="btn_risk"):
...
if st.button("Generate Status Report", type="primary", key="btn_report"):
    ...
    health_icon = {"GREEN": "🟢", "YELLOW": "🟡", "RED": "🔴"}.get(evm["health_status"], "⚪")

    def _cpi_label(v: float) -> str:
        if v >= 1.0:
            return "Under budget ✅"
        if v >= 0.9:
            return "Slightly over budget ⚠️"
    ...
    def _spi_label(v: float) -> str:
        ...
        if v >= 0.9:
            return "Slightly behind schedule ⚠️"
        return "Significantly behind schedule 🔴"

    report = f"""
## Executive Status Report

**Overall Health:** {health_icon} {evm["health_status"]}
...
# 3. DELAY SIMULATOR
# ─────────────────────────────────────────────────────────────────────────────
st.subheader("⏱️ Delay Simulator")
st.markdown(
...
# Load task list for the selectbox
try:
...
if not task_options:
    ...
    if st.button("Simulate Delay", type="primary", key="btn_delay"):
        task_data = next(
        ...
        if task_data is None:
            ...
            if is_critical:
                st.warning(
                    f"⚠️ '{selected_task}' is on the **critical path** — "
                    "it has zero float and any delay has maximum project impact."
                )
            elif float_available > 0:
                st.info(
                    f"ℹ️ '{selected_task}' has {float_available} days of float. "
